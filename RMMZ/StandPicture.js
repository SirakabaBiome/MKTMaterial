 ...
 * @author 白樺まこと
 * @base PictureAnimation
 * @target MZ
 * @url https://diary.sirakababiome.com/2020/02/mvStand.html
 *
 * @help
 *　このプラグインは
...
(function() {
  ...
  var paramKeyT = 0;
	...
	const _PluginManager_registerCommand = PluginManager.registerCommand;
	PluginManager.registerCommand(pluginName,"show",function(args) {
		interpreter = this;
		switch ( args.showPosition ) {

			case 'L':
				if ( mktSPLB > 0 ) {
					clearInterval(partBlinkSetL);
				}
				if ( args.increment1cell > 1 ) {
					mktSPLB = 1;
				} else {
					mktSPLB = -1;
				}
				costume = args.cosName;
				if ( mktSPLN == args.PictureName) {
					mktChange = true;
				} else {
					mktChange = false;
				}
        ...
        paramKeyT = JSON.parse(paramKey[i]);
          if ( String(args.PictureName) == paramKeyT['FileName'] ) {
            ...
            break;
					...
					paramLLR = 1;
					mktSPLN = args.PictureName;
					mktStundPictureShow(0,mktPY,args.PictureName,args.increment1,args.increment2,mktLPID,args.increment1cell,args.increment2cell,partIndexL);
					break;
        ...
        mktSPCN = args.PictureName;
				var autoET = $gameSwitches.value(autoEre);
				if ( autoET != true ) {
					if ( mktSPRN == args.PictureName ) {
						mktHide(mktRPID,mktRX + stX,"R",partIndexR);
					} else if ( mktSPLN == args.PictureName ) {
        ...
        paramKeyT = JSON.parse(paramKey[i]);
          if ( String(args.PictureName) == paramKeyT['FileName'] ) {
	          mktSPCCN = paramKeyT['CharacterName'];
				...
				mktSPCN = args.PictureName;
				paramLLR = 3;
				mktStundPictureShow(mktCX,mktPY,args.PictureName,args.increment1,args.increment2,mktCPID,args.increment1cell,args.increment2cell,partIndexC);
          break;
		        ...
		        if ( mktSPLN == args.PictureName ) {
							mktHide(mktLPID,0 - stX,"L",partIndexL);
		        } else if ( mktSPCN == args.PictureName ) {
							mktHide(mktCPID,mktCX,"C",partIndexC)
		        }
					}
	        for (var i = 0; i < paramKey.length; i++) {
	        paramKeyT = JSON.parse(paramKey[i]);
	          if ( String(args.PictureName) == paramKeyT['FileName'] ){
	            mktSPRCN = paramKeyT['CharacterName'];
					...
					mktSPRN = args.PictureName;
					paramLLR = 2;
	        mktStundPictureShow(mktRX,mktPY,args.PictureName,args.increment1,args.increment2,mktRPID,args.increment1cell,args.increment2cell,partIndexR);
	...
	PluginManager.registerCommand(pluginName,"move",function(args) {
	...
	PluginManager.registerCommand(pluginName,"finish",function(args) {
...
//ピクチャの表示
  function mktStundPictureShow(x, y, pictureName,index,index2,pictureID,partLS,partBS,partIndex){
		...
		if ( paramL != false ) {
			if ( paratL > 0 ) {
    		var standPLip = {cellNumber:partLS,fade:paramL,direction:'H',frameNumber:'0'};
				PluginManager.callCommand(interpreter'PictureAnimation','INIT',function(standPLip));
		...
		if ( paramB != false ) {
			if ( paratB > 0) {
    		var standPBlink = {cellNumber:partBS,fade:paramB,direction:'H',frameNumber:'0'};
				PluginManager.callCommand('PictureAnimation','INIT',function(standPBlink));
    		// var standPBlink = [String(partBS),String(paramB),'横','0'];
				// $gameScreen.setPicturesAnimation(String(partBS),'0','H',String(paramB));
